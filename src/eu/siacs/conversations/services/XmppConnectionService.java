...
public class XmppConnectionService extends Service implements OnPhoneContactsLoadedListener {
	...
	private void sendInitialPresence(Account account){
		try{
		if(as == null) 
			as = AccountStatus.loadSerializedObject(getApplicationContext());
		if(as != null){
			String user = account.getUsername() + "@ximpim.com";
			String status = "";
			try{
			status = as.accountStatus.get(user);
			}
			...
			int position = 0;
			try{
			position = as.accountStatusPos.get(user);
			}
			catch(NullPointerException ex){
				position =0;
			}
			PresencePacket presencePacket = new PresencePacket();
			presencePacket.addChild("status").setContent(status);
	...
	private OnAvatarUpdate mOnAvatarUpdate = null;

	private int mucRosterChangedListenerCount = 0;
	private int mucAvatarChangedListenerCount = 0;
	...
	public Account getAccount(String accountName) {
		for( int i=0;i<this.accounts.size();i++){
			String localPart = accounts.get(i).getJid().getLocalpart();
			String domainPart = accounts.get(i).getJid().getDomainpart();
			String userId = localPart+"@" + domainPart ;
			if(userId.equalsIgnoreCase(accountName))
	...
	public void setOnAvatarUpdateListener(OnAvatarUpdate listener) {
		synchronized (this) {
			if (checkListeners()) {
				switchToForeground();
			}
			this.mOnAvatarUpdate = listener;
			if (this.mucAvatarChangedListenerCount < 2) {
				this.mucAvatarChangedListenerCount++;
			}
		}
	}
	...
	public void publishAvatar(final Account account,
		...
		if (avatar != null) {
			...
			final IqPacket vCardPacket = this.mIqGenerator.publishvCardAvatar(avatar);
			vCardPacket.setFrom(account.getJid());
			this.sendIqPacket(account, vCardPacket, new OnIqPacketReceived() {
				...
				public void onIqPacketReceived(Account account, IqPacket packet) {
					...
					if (packet.getType() == IqPacket.TYPE.RESULT) {
						...
						Log.i(Utils.TAG, "vCard Result - "+packet.toString());
					PresencePacket ppacket = new PresencePacket();
					ppacket.setFrom(account.getJid());
					Element vcard = ppacket.addChild("x", "vcard-temp:x:update");
					vcard.addChild("photo").setContent(avatar.sha1sum);
					Log.i(Utils.TAG, vcard.toString());
					sendPresencePacket(account, ppacket);
					}
				}
			});
			final IqPacket packet = this.mIqGenerator.publishAvatar(avatar);
			packet.setFrom(account.getJid());
			packet.setId("publish1");
...
*/									callback.success(avatar);
	...
	public void fetchvCard(Account account,final Contact contact){
		final IqPacket vCardPacket = this.mIqGenerator.publishvCardPacket(contact);
		vCardPacket.setFrom(account.getJid());
		this.sendIqPacket(account, vCardPacket, new OnIqPacketReceived() {
			...
			public void onIqPacketReceived(Account account, IqPacket packet) {
				...
				if (packet.getType() == IqPacket.TYPE.RESULT) {
					//Log.i(Utils.TAG, "vCard Result - "+packet.toString());
					String base64 = Utils.parseAvatar(packet.toString());
					Avatar avatar = new Avatar();
					avatar.owner = contact.getJid();
					avatar.height = Config.AVATAR_SIZE ;
					avatar.width = Config.AVATAR_SIZE ;
					avatar.image= base64;
					if(base64.length()>0)
					if (getFileBackend().save(avatar,true,contact.getJid().getLocalpart())) {
						if (account.getJid().toBareJid().equals(avatar.owner)) {
							if (account.setAvatar(contact.getJid().getLocalpart())) {
								databaseBackend.updateAccount(account);
							}
							...
							contact.setAvatar(contact.getJid().getLocalpart()+".png");
							databaseBackend.updateContact(contact);
							getAvatarService().clear(contact);
							databaseBackend.getContacts();
							updateConversationUi();
							updateRosterUi();

						}
						updateAvatarUi(Utils.parseAvatar(packet.toString()));
					...
					//Log.i(Utils.TAG, "vCard Result - "+Utils.parseAvatar(packet.toString()));
				}
			}
		});
	...
	public void updateAvatarUi(String Avatar) {
		if (mOnAvatarUpdate != null) {
			mOnAvatarUpdate.onAvatarUpdate(Avatar);
		}
	}
	...
	public interface OnAvatarUpdate {
		public void onAvatarUpdate(String Avatar);
	}
