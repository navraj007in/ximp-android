...
public class IqGenerator extends AbstractGenerator {
	...
	public IqPacket publishvCardAvatar(Avatar avatar) {
		final IqPacket packet = new IqPacket(IqPacket.TYPE.SET);
		packet.setFrom(avatar.owner);
		...
		type.setContent(avatar.type);
		binval.setContent(avatar.image.trim().substring(0, avatar.image.trim().length()-1));
		
		Log.i(Utils.TAG, packet.toString());
		return packet;
	}

	public IqPacket publishvCardPacket(Contact contact) {
		final IqPacket packet = new IqPacket(IqPacket.TYPE.GET);
		packet.setId("v3");
