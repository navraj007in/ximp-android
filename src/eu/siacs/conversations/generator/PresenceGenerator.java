...
public class PresenceGenerator extends AbstractGenerator {
	...
	public PresencePacket sendPresence(Account account,PresencePacket presencePacket) {
		presencePacket.setFrom(account.getJid());

		String sig = account.getPgpSignature();
		if (sig != null) {
			presencePacket.addChild("status").setContent("dnd");
			presencePacket.addChild("x", "jabber:x:signed").setContent(sig);
		}
		String capHash = getCapHash();
		if (capHash != null) {
			Element cap = presencePacket.addChild("c",
					"http://jabber.org/protocol/caps");
			cap.setAttribute("hash", "sha-1");
			cap.setAttribute("node", "http://conversions.im");
			cap.setAttribute("ver", capHash);
		}
		Log.i(Utils.TAG, presencePacket.toString());
		presencePacket.toServer(account);
